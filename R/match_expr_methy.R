...
match_expr_methy <- function(gene_express_data,gene_methy){
  ...
  for (i in 1:length(target_genes)) {
    one_exp_methy <- c(as.numeric(as.character(full_expr_gene[rownames(full_expr_gene)==target_genes[i],])),
                      as.numeric(as.character(gene_methy[rownames(gene_methy)==target_genes[i],])))
    gene_exp_methy <- rbind(gene_exp_methy,one_exp_methy)
  }
  rownames(gene_exp_methy) <- target_genes
  colnames(gene_exp_methy) <- c(colnames(full_expr_gene),colnames(gene_methy))
